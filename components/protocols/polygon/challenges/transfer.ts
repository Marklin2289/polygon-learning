...
const transfer = async () => {
  try {
    ...
    const transaction = {
      ...
      nonce: await provider.getTransactionCount(send_account, 'latest'),
      gasLimit: ethers.utils.hexlify(100000),
      gasPrice: gas_price,
    };

    const hash = await provider.getSigner().sendTransaction(transaction); //sendTransaction()
    const receipt = await hash.wait(); // wait for transaction
